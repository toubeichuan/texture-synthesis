...
from lib.diffusion_helper_new_version2 import (
    get_controlnet_depth,
    apply_controlnet_depth_RefAttn,
    get_inpainting,
...
--output_dir outputs_labubu_generate_texture_labubu_lockedview0AndCOR_test25WithCor_1.4
--obj_file cat.obj
...
--style_img data/texture_images/round_bird/ipadapter_result_00_labubu.png
...
def _get_apply_controlnet_optional_kwargs(reference_image=None):
    """
    兼容新旧 diffusion helper：
    - 新版支持 `reference_image`
    - 旧版不支持时自动跳过，避免 TypeError
    """
    optional_kwargs = {}
    try:
        sig = inspect.signature(apply_controlnet_depth_RefAttn)
        if "reference_image" in sig.parameters and reference_image is not None:
            optional_kwargs["reference_image"] = reference_image
...
def _get_num_principle_views(args):
    camera_preset = _get_camera_preset(args)
    if camera_preset is None:
        return len(VIEWPOINTS[6]["sector"])
    return len(VIEWPOINTS[camera_preset]["sector"])


def _resolve_style_mask_path(style_img_path, style_mask_path=""):
    if style_mask_path:
        if os.path.isfile(style_mask_path):
            return style_mask_path
        raise FileNotFoundError(
            "显式指定的 style mask 不存在: {}".format(style_mask_path)
        )

    style_img_root, style_img_ext = os.path.splitext(style_img_path)
    candidate_exts = [style_img_ext, ".png", ".jpg", ".jpeg", ".webp", ".bmp"]
    tried_paths = []
    for ext in candidate_exts:
        candidate_path = style_img_root + "_mask" + ext
    ...
    raise FileNotFoundError(
        "未找到 style image 对应的 mask 文件。\n"
        "style_img: {}\n"
        "已尝试:\n- {}\n"
        "你可以补充一个同名 sidecar mask（如 *_mask.png / *_mask.jpg），"
        "或者通过 --style_mask 显式指定。".format(
            style_img_path,
            "\n- ".join(tried_paths),
        )
    )
...
def init_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="如果设置，则将该图片直接作为 view 0 的生成结果，而不是把它当作扩散输入图。",
    )
    # IP-Adapter arguments
    parser.add_argument("--style_img", type=str, required=True)
    parser.add_argument("--style_mask", type=str, default="")
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        print("[INFO] 未提供命令行参数，使用 generate_texture.py 内置 DEFAULT_RUN_CMD 默认参数运行。")
    args = init_args()
    camera_preset = _get_camera_preset(args)

    os.environ.setdefault("EASITEX_REF_ATTN_RUNTIME_CHECKS", "1")
    os.environ.setdefault("EASITEX_REF_ATTN_RUNTIME_CHECKS_MAX_STEPS", "6")
    print(
        "[RefAttnCheck] 已开启 reference attention 运行期验证日志。"
        "如需关闭，可设置 EASITEX_REF_ATTN_RUNTIME_CHECKS=0。"
    )
    ...
    frozen_update_texture = torch.zeros_like(exist_texture)
    ...
    if args.style_img != "":
        ...
        style_mask_path = _resolve_style_mask_path(args.style_img, args.style_mask)
        print("=> using style mask:", style_mask_path)
        style_image = ImageUtils.load_image(args.style_img)
        style_image_mask = ImageUtils.load_image(style_mask_path)
        style_image, style_image_mask = ImageUtils.recenter_object(style_image, style_image_mask, 512, 0.9)  # NOTE: Hardcoded

        shared_reference_np = ImageUtils.swap_background(style_image, style_image_mask, args.style_img_bg_color)
        shared_reference_image = Image.fromarray(shared_reference_np).convert("RGB")

        aug_style_image = cv2.resize(shared_reference_np, (224, 224), interpolation=cv2.INTER_AREA)
    ...
    NUM_PRINCIPLE = _get_num_principle_views(args)
    print(
        "=> using {} principle camera preset with {} viewpoints".format(
            camera_preset if camera_preset is not None else "default",
            NUM_PRINCIPLE,
        )
    )
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        if view_idx == 0 and args.override_rendering_0:
            if not os.path.isfile(args.override_rendering_0):
                raise FileNotFoundError(
                    "--override_rendering_0 指定的文件不存在: {}".format(args.override_rendering_0)
                )
            ...
            print("=> use override image as direct generated result for view 0:", args.override_rendering_0)
        ...
        if view_idx == 0 and override_img is not None:
            generate_image = override_img
            generate_image_before = init_image
            generate_image_after = override_img
            print("=> skip view-0 new-stage diffusion and use override image as final result")
        else:
            generate_image, generate_image_before, generate_image_after = apply_controlnet_depth_RefAttn(controlnet, ddim_sampler, 
                ...
                **_get_apply_controlnet_optional_kwargs(shared_reference_image))
        ...
        exist_texture_before = exist_texture.clone() if view_idx == 0 else None
        ...
        if view_idx == 0:
            frozen_update_texture = torch.maximum(
                frozen_update_texture,
                (exist_texture > exist_texture_before).float()
            )
            print(
                "=> freeze {} UV texels from initial view 0 against all future update stages".format(
                    int(frozen_update_texture.sum().item())
                )
            )
        ...
        # 1.2.3. re: render
        # Recompute masks after projecting the new region. The update pass must
        # use these fresh masks; otherwise view1 keeps the pre-generation mask
        # and can incorrectly skip update.
        (
            view_score,
            renderer, cameras, camera_pos, fragments,
            init_image, normal_map, depth_map,
        ...
        # Force an update pass for every non-initial generation view so that
        # all later viewpoints get one refinement round regardless of mask size.
        update_pixels = float(update_mask_tensor.sum().item())
        all_pixels = max(float(all_mask_tensor.sum().item()), 1.0)
        update_ratio = update_pixels / all_pixels
        if view_idx != 0 and not args.no_update and update_mask_tensor.sum() > 0:
            print(
                "=> force update {} pixels for view {} (ratio {:.2%})".format(
                    int(update_pixels), view_idx, update_ratio
                )
            )
            update_mask_image.save(os.path.join(mask_image_dir, "{}_update_actual_generate.png".format(view_idx)))
            update_mask_image.save(os.path.join(mask_image_dir, "{}_update_actual_project.png".format(view_idx)))

            diffused_image, diffused_image_before, diffused_image_after = apply_controlnet_depth_RefAttn(controlnet, ddim_sampler, 
    ...
    if args.update_steps > 0:
        ...
        for view_idx in range(args.update_steps):
            ...
            if not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.05:
                ...
                update_image, update_image_before, update_image_after = apply_controlnet_depth_RefAttn(controlnet, ddim_sampler, 
                    init_image.convert("RGBA"), prompt, args.update_strength, args.ddim_steps,
