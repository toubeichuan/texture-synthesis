...
from lib.diffusion_helper_refcover_v2 import (
    get_controlnet_depth,
    get_controlnet_inpaint_pipe_sd15_refattn,
    get_inpainting,
    apply_inpaintcontrolnetrefattn_depth,
...
--output_dir outputs_labubu_fix
--obj_file cat.obj
--prompt "kawaii chibi toy character, fluffy beige rabbit hood, big sparkling pink eyes, cute mischievous smile with small teeth, pastel toned, clean stitching details, soft fabric texture, blue pinstripe shirt, denim shorts, tiny sneakers"
...
--update_strength 0.5
--view_threshold 0.1
--update_min_ratio 0.01
--patch_min_ratio 0.001
--blend 0.1
--dist 0.8
--num_viewpoints 1
...
def init_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Minimum update-mask ratio required to run update diffusion.",
    )
    parser.add_argument(
        "--patch_min_ratio",
        type=float,
        default=0.001,
        help="Minimum new-mask ratio required to run patch diffusion in refinement stage.",
    )
    ...
    parser.add_argument(
        "--use_sd2_inpaint_new_stage",
        type=str2bool,
        default=True,
        help="If True, use SD2 StableDiffusionInpaintPipeline to fill the NEW/generate mask in generation stage (view>0). Update/refinement stage remains unchanged.",
    )
    parser.add_argument(
        "--override_rendering_0",
        type=str,
        default="",
        help="If set, replace generate/rendering/0.png with this image (path). Useful when you already have a colored view-0 image.",
    )
    parser.add_argument(
        "--skip_view0_new_stage",
        type=str2bool,
        default=True,
        help="When override_rendering_0 is set, skip the 'new' diffusion stage for view 0 and directly back-project the provided image.",
    )
    parser.add_argument(
        "--skip_view0_update_stage",
        type=str2bool,
        default=True,
        help="When override_rendering_0 is set, skip the 'update/refinement' diffusion stage whenever the selected viewpoint is view-0 (selected_view_id == 0).",
    )
    ...
    parser.add_argument("--ip_adapter_strength", type=float, default=0.5)
    ...
    # device parameters
    parser.add_argument("--tex_resolution", type=str, choices=["1k", "3k"], default="1k") # CHANGED default to 1k for stability
    ...
    if args.tex_resolution == "3k":
        ...
        setattr(args, "fragment_k", 1)
        # CHANGED: Force 512 for better SD1.5 compatibility if needed, but 768 is also okay for some models. 
        # Using 768 as per original logic, but be aware SD1.5 is native 512.
        setattr(args, "image_size", 768) 
        setattr(args, "uv_size", 1000)

    return args

# [FIX] Helper function to normalize depth map for ControlNet
def process_depth_map(depth_map_tensor, image_size, device):
    """
    Normalizes a raw PyTorch3D depth map (B, H, W) to a ControlNet-compatible
    Format: 0-255 uint8 numpy array, (H, W, 3), where Near=255(White), Far=0(Black).
    """
    # Take first batch item
    depth = depth_map_tensor[0] # [H, W]
    
    # Create mask for valid depth (assuming background is 0 or -1, adapt if needed)
    # PyTorch3D usually renders background as -1 or far plane. 
    # Here we assume > 0 is valid object.
    valid_mask = depth > 0
    ...
    # Normalize: 0 = near (min depth), 1 = far (max depth)
    depth_norm[valid_mask] = (depth[valid_mask] - d_min) / (d_max - d_min + 1e-6)
    
    # Invert for ControlNet: White (1) = Near, Black (0) = Far
    depth_inverted = 1.0 - depth_norm
    depth_inverted[~valid_mask] = 0.0 # Background is black
    
    # Convert to numpy uint8 0-255
    depth_np = (depth_inverted.cpu().numpy() * 255).astype(np.uint8)
    
    # Stack to 3 channels
    control_image = np.stack([depth_np] * 3, axis=-1)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        sys.argv.extend(shlex.split(DEFAULT_RUN_CMD))
        print("[INFO] No args provided. Using DEFAULT_RUN_CMD.")
    ...
    if args.style_img != "":
        ...
        ip_adapter_weights = ipadapter_model.get("ip_adapter", None)
        if ip_adapter_weights is None:
            raise ValueError(f"Invalid IP-Adapter checkpoint, missing key 'ip_adapter': {ipadapter_ckpt_path}")
        ...
        # [FIX] Safer mask loading
        mask_path = img_path_split[0] + "_mask" + img_path_split[1] + img_path_split[2]
        if os.path.exists(mask_path):
            style_image_mask = ImageUtils.load_image(mask_path)
        else:
            print(f"[WARNING] Style mask not found at {mask_path}. Using full white mask.")
            style_image_mask = np.ones((style_image.shape[0], style_image.shape[1]), dtype=np.uint8) * 255
            
        style_image, style_image_mask = ImageUtils.recenter_object(style_image, style_image_mask, 512, 0.9)

        aug_style_image_ref = ImageUtils.swap_background(
            style_image, style_image_mask, args.style_img_bg_color
        )
        aug_style_image_ref = Image.fromarray(aug_style_image_ref).convert("RGB")

        aug_style_image = cv2.resize(
            np.array(aug_style_image_ref), (224, 224), interpolation=cv2.INTER_AREA
        )
        ...
        inpaint_pipe_sd15 = get_controlnet_inpaint_pipe_sd15_refattn(
            device=DEVICE,
            controlnet_cond=args.controlnet_cond,
            controlnet_strength=float(args.controlnet_strength),
            ip_adapter_strength=float(args.ip_adapter_strength),
            ip_adapter_path=args.ip_adapter_path,
            num_tokens=int(image_prompt_embeds.shape[1]),
    ...
    if args.use_objaverse:
        NUM_PRINCIPLE = len(VIEWPOINTS["objaverse"]["sector"])
    elif args.use_shapenet:
        NUM_PRINCIPLE = len(VIEWPOINTS["shapenet"]["sector"])
    else:
        NUM_PRINCIPLE = 6
    ...
    # [FIX] Optimization: wrap generation loop in no_grad
    with torch.no_grad():
        for view_idx in range(NUM_PRINCIPLE):
            ...
            if view_idx == 0 and args.override_rendering_0:
                ...
                print(f"=> override rendering/0.png with: {args.override_rendering_0}")
            ...
            if view_idx == 0 and args.override_rendering_0 and args.skip_view0_new_stage:
                generate_image = init_image
                generate_image_before = init_image
                generate_image_after = init_image
                print("=> skip view-0 new-stage diffusion (using override image).")
            else:
                if inpaint_pipe_sd15 is None:
                    raise RuntimeError("inpaint_pipe_sd15 is not initialized; check IP-Adapter loading.")

                # [FIX] Proper depth map normalization
                control_image_np = process_depth_map(depth_maps_tensor.detach(), args.image_size, DEVICE)

                generate_image, generate_image_before, generate_image_after = apply_inpaintcontrolnetrefattn_depth(
                    inpaint_pipe_sd15,
                    ...
                    DEVICE,
                    args.blend,
                    reference_image=aug_style_image_ref,
            ...
            print("=> saved generate image to: {}".format(os.path.join(inpainted_image_dir, "{}.png".format(view_idx))))
            ...
            if (
                ...
                and update_mask_tensor.sum() / (all_mask_tensor.sum()) > float(args.update_min_ratio)
            ):
                print("=> update {} pixels for view {}".format(update_mask_tensor.sum().int(), view_idx))
                
                # [FIX] Proper depth normalization + detach
                control_image_np = process_depth_map(depth_maps_tensor.detach(), args.image_size, DEVICE)

                diffused_image, diffused_image_before, diffused_image_after = apply_inpaintcontrolnetrefattn_depth(
                    inpaint_pipe_sd15,
                    init_image.convert("RGB"),
                    prompt,
    ...
    if args.update_steps > 0:
        ...
        # [FIX] Wrap update loop in no_grad
        with torch.no_grad():
            for view_idx in range(args.update_steps):
                ...
                if len(selected_view_ids) == 0:
                    print("=> No suitable viewpoint found (selected_view_ids is empty). Breaking.")
                    break
                ...
                (
                    ...
                    selected_view_ids[-1], view_idx, view_punishments, 
                    similarity_texture_cache, exist_texture,
                    mesh, edge_mesh, faces, new_verts_uvs,
                    args.image_size, args.fragment_k,
                    init_image_dir, mask_image_dir, normal_map_dir, depth_map_dir, similarity_map_dir,
                ...
                if (
                    args.override_rendering_0
                    and args.skip_view0_update_stage
                    and selected_view_ids[-1] == 0
                ):
                    print("=> skip view-0 update-stage diffusion (using rendered init_image).")
                    update_image = init_image
                    update_image_before = init_image
                    update_image_after = init_image
                    old_mask_tensor += update_mask_tensor
                    update_mask_tensor[update_mask_tensor == 1] = 0
                    ...
                    if (
                        args.use_patch
                        and (not args.no_repaint)
                        and generate_mask_tensor.sum() > 0
                        and generate_mask_tensor.sum() / (all_mask_tensor.sum()) > float(args.patch_min_ratio)
                    ):
                        print("=> patch {} pixels for view {}".format(generate_mask_tensor.sum().int(), view_idx))
                        
                        # [FIX] detach & normalize
                        control_image_np = process_depth_map(depth_maps_tensor.detach(), args.image_size, DEVICE)

                        patch_image, patch_image_before, patch_image_after = apply_inpaintcontrolnetrefattn_depth(
                            inpaint_pipe_sd15,
                            init_image.convert("RGB"),
                            prompt,
                            args.new_strength,
                        ...
                        patch_image.save(os.path.join(inpainted_image_dir, "{}_new.png".format(view_idx)))

                        # Back-project patch results
                        init_texture, patch_project_mask_image, exist_texture = backproject_from_image(
                            mesh, faces, new_verts_uvs, cameras,
                            patch_image, generate_mask_image, generate_mask_image, init_texture, exist_texture,
                            args.image_size * args.render_simple_factor, args.uv_size, args.fragment_k,
                            DEVICE
                        )
                        patch_project_mask_image.save(
                            os.path.join(mask_image_dir, "{}_new_project.png".format(view_idx))
                        )
                        ...
                        # [FIX] Update init_image so the Update Step sees the patched content!
                        # Use generate_mask to paste the patch onto the original render
                        # Because backprojection is lossy, we ideally re-render, but compositing is faster and usually sufficient here.
                        init_image = Image.composite(patch_image, init_image, generate_mask_image)
                    ...
                    if (
                        ...
                        update_image, update_image_before, update_image_after = apply_inpaintcontrolnetrefattn_depth(
                            inpaint_pipe_sd15,
                            init_image.convert("RGB"),
                            prompt,
                            args.update_strength,
