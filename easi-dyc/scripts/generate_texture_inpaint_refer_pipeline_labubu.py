...
--output_dir outputs_labubu_referpipeline01_noIP
--obj_file cat.obj
...
--tex_resolution 1k
--use_shapenet
--override_rendering_0 data/texture_images/round_bird/ipadapter_result_00_labubu.png
...
def init_args():
    ...
    # Trainable Reference-Attention adapter options (LoRA-style lightweight finetune)
    parser.add_argument("--refattn_trainable", type=str2bool, default=False)
    parser.add_argument("--refattn_ckpt", type=str, default="")
    parser.add_argument("--refattn_ckpt_strict", type=str2bool, default=True)
...
if __name__ == "__main__":
    ...
    # SD1.5 ControlNet Inpaint pipeline (with reference attention)
    ...
    if args.style_img != "":
        ...
        # Reference image for "reference attention" (keep a reasonably high resolution; actual resize happens per-view)
        ...
        # CLIP/IP-Adapter path still uses 224x224
        ...
        # init SD1.5 ControlNet Inpaint pipeline (used by reference attention)
    ...
    # Optional: switch to trainable ref-attn adapters and load finetuned weights.
        # This must happen right after pipeline creation, before any `pipe(...)` call,
        # so every denoising step uses the same wrapped UNet/attn processors.
# Optional: switch to trainable ref-attn adapters and load finetuned weights.
        # This must happen right after pipeline creation, before any `pipe(...)` call,
        # so every denoising step uses the same wrapped UNet/attn processors.
        if args.refattn_trainable or (args.refattn_ckpt is not None and len(args.refattn_ckpt) > 0):
            ensure_trainable_ref_unet_wrapped(
                inpaint_pipe_sd15,
            ...
            print(
                "=> enabled trainable reference attention adapters "
                f"(rank={args.refattn_rank}, alpha={args.refattn_lora_alpha}, "
                f"dropout={args.refattn_dropout}, init_ref_scale={args.refattn_init_ref_scale})"
            )
            if args.refattn_ckpt is not None and len(args.refattn_ckpt) > 0:
                if not os.path.isfile(args.refattn_ckpt):
                    raise FileNotFoundError(f"refattn_ckpt not found: {args.refattn_ckpt}")
                load_info = load_trainable_refattn(
                    inpaint_pipe_sd15.unet,
                    args.refattn_ckpt,
                    strict=bool(args.refattn_ckpt_strict),
                )
                print(
                    "=> loaded ref-attn ckpt: "
                    f"{args.refattn_ckpt} | "
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        if view_idx == 0 and args.override_rendering_0 and args.skip_view0_new_stage:
            ...
            # Reference attention + ControlNet inpaint (SD1.5)
        ...
        print("=> saved generate image before to: {}".format(os.path.join(inpainted_image_dir, "{}_before.png".format(view_idx))))
        print("=> saved generate image after to: {}".format(os.path.join(inpainted_image_dir, "{}_after.png".format(view_idx))))    
