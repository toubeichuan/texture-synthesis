...
def init_args():
    ...
    parser.add_argument("--view_threshold", type=float, default=0.03)
...
if __name__ == "__main__":
    ...
    if args.style_img != "":
        ...
        # init SD1.5 ControlNet Inpaint pipeline (IP-Adapter only)
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        if view_idx == 0 and args.override_rendering_0 and args.skip_view0_new_stage:
            ...
            # IP-Adapter + ControlNet inpaint (SD1.5)
            ...
            generate_image, generate_image_before, generate_image_after = apply_controlnet_inpaint(
                ...
                DEVICE,
                args.blend,
                control_image_np=control_image_np,
        ...
        if not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.05:
            ...
            diffused_image, diffused_image_before, diffused_image_after = apply_controlnet_inpaint(
                ...
                DEVICE,
                args.blend,
                control_image_np=depth_maps_tensor.permute(1, 2, 0).repeat(1, 1, 3).cpu().numpy(),
                controlnet_conditioning_scale=float(args.controlnet_strength),
                pos_img_prompt_embeds=image_prompt_embeds,
                neg_img_prompt_embeds=negative_image_prompt_embeds,
            )
