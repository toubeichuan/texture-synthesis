...
from lib.diffusion_helper import (
    get_inpainting,
    apply_controlnet_inpaint,
    get_controlnet_inpaint_pipe_sd15,
...
# 直接运行：
#   python scripts/generate_texture_inpaint.py
# 会自动使用下面这串默认参数（可按需改成你自己的 run.sh 配置）。
# 如果你传了命令行参数，则命令行参数会覆盖这里的默认值。
DEFAULT_RUN_CMD = r"""
--input_dir data/meshes/cat
--output_dir outputs_cat_new_shapenet_withallInpaint_1.4
--obj_file cat.obj
--prompt "American Shorthair,yellow-green eyes, fluffy paws, A cat with black and silver stripes, white background,photorealistic, high quality texture"
--style_img data/texture_images/round_bird/cat2.jpg
--style_img_bg_color 255 255 255
--ip_adapter_path ./ip_adapter
--ip_adapter_strength 2
...
--tex_resolution 1k
--use_shapenet
--override_rendering_0 data/texture_images/round_bird/ipadapter_result_00_cat.png
...
def init_args():
    ...
    # Optional: override the first rendered view with a pre-colored image (skip dummy / skip first diffusion if desired)
    parser.add_argument(
...
if __name__ == "__main__":
    # 如果没有任何命令行参数：使用 DEFAULT_RUN_CMD 作为默认参数，让脚本可直接运行。
    if len(sys.argv) == 1:
        # 解析多行命令字符串（兼容引号）
        sys.argv.extend(shlex.split(DEFAULT_RUN_CMD))
        print("[INFO] 未提供命令行参数，使用 generate_texture_inpaint.py 内置 DEFAULT_RUN_CMD 默认参数运行。")
    ...
    "DEBUG_MASKED_IMAGE_DIR",
    os.path.join(output_dir, "debug", "masked_image")
    )
    ...
    # NOTE: 不再加载 SD2 inpainting（避免离线下载问题 / 也不符合“SD1.5 diffusers ControlNet inpaint”的目标）。
    # diffusers SD1.5 ControlNet InpaintPipeline 会在拿到 IP-Adapter embeds 后再初始化。
    ...
    if args.style_img != "":
        ...
        # ===== diffusers: SD1.5 ControlNet InpaintPipeline + IP-Adapter processors =====
        # num_tokens 必须与 image_prompt_embeds 的 token 维度一致，否则 cross-attn 拼接长度对不上
        inpaint_pipe_sd15 = get_controlnet_inpaint_pipe_sd15(
            device=DEVICE,
    ...
    # Debug directory for inpaint function debugging
    debug_dir = os.path.join(generate_dir, "debug")
    ...
    # 动态获取principle视角数量：objaverse=72, shapenet=10, 其他=6
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        # Optional: override the first rendered image with a user-provided colored image.
        # We still keep depth/normal/masks from the real rendering, and only replace init_image.
        if view_idx == 0 and args.override_rendering_0:
            ...
            # make sure generate/rendering/0.png is exactly the override image
        ...
        if view_idx == 0 and args.override_rendering_0 and args.skip_view0_new_stage:
            # Already colored: directly use init_image as "generated" image for back-projection.
            ...
            #if (inpainting_sd2 is not None) and (view_idx > 0):
                # SD2 inpaint: only fill the NEW/generate mask, strictly preserve known/old texture regions.
            # For debug/visualization, keep mask should represent EXIST region (= old ∪ update),
            # matching the saved *_exist.png from render_one_view_and_build_masks.
            _old_np = (np.array(keep_mask_image.convert("L")) > 127)
            _upd_np = (np.array(update_mask_image.convert("L")) > 127)
            exist_mask_image = Image.fromarray(((_old_np | _upd_np).astype(np.uint8) * 255)).convert("L")
            generate_image, generate_image_before, generate_image_after = apply_controlnet_inpaint(
                inpaint_pipe_sd15,
                ...
                control_image_np=depth_maps_tensor.permute(1, 2, 0).repeat(1, 1, 3).detach().cpu().numpy(),
                controlnet_conditioning_scale=float(args.controlnet_strength),
                strict_compose=True,
                debug_dir=os.path.join(debug_dir, f"view_{view_idx}_new"),
                # Fix: make the *actual* inpaint/noise region exclude EXIST pixels (exist mask = old ∪ update)
                exclude_keep_from_generate_mask=True,
                # Fix: depth_pad/background semantics only apply for depth maps, not canny/edges.
                merge_background=(args.controlnet_cond == "depth"),
                # Fix: dilation tends to eat seams for canny edges; keep it only for depth.
                dilate_mask=(args.controlnet_cond == "depth"),
            )
            # else:
            #     # fallback: original ControlNet RePaint (view0 or when SD2 inpaint disabled)
            #     generate_image, generate_image_before, generate_image_after = apply_controlnet_depth(
            #         controlnet,
            #         ddim_sampler,
        ...
        if not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.01:
            print("=> update {} pixels for view {}".format(update_mask_tensor.sum().int(), view_idx))
            # 用 apply_controlnet_inpaint 替换 apply_controlnet_depth，但保持相同的上色/合成规则：
            # - 只在 update_mask_image 区域生成
            # - 其它区域严格保留 init_image（strict_compose=True）
            diffused_image, diffused_image_before, diffused_image_after = apply_controlnet_inpaint(
                inpaint_pipe_sd15,
                init_image.convert("RGB"),
                prompt,
                ...
                controlnet_conditioning_scale=float(args.controlnet_strength),
                strict_compose=True,
                debug_dir=os.path.join(debug_dir, f"view_{view_idx}_update"),
            )
    ...
    if args.update_steps > 0:
        ...
        # Debug directory for inpaint function debugging (refinement stage)
        debug_dir_refine = os.path.join(update_dir, "debug")
        os.makedirs(debug_dir_refine, exist_ok=True)
        ...
        for view_idx in range(args.update_steps):
            ...
            # Optional: skip view-0 refinement diffusion when using an override colored view-0 image.
            # Here "view-0" refers to the selected viewpoint id (selected_view_ids[-1] == 0), not the refinement iteration index.
            if (
                args.override_rendering_0
                and args.skip_view0_update_stage
                and len(selected_view_ids) > 0
                ...
                # keep masks consistent: treat as 'nothing to update' to avoid projection side effects
            ...
            elif not args.no_update and update_mask_tensor.sum() > 0 and update_mask_tensor.sum() / (all_mask_tensor.sum()) > 0.05:
                print("=> update {} pixels for view {}".format(update_mask_tensor.sum().int(), view_idx))
                # 用 apply_controlnet_inpaint 替换 apply_controlnet_depth，并保持相同的 mask 规则：
                # - update_mask_image: 需要更新/上色的区域
                # - old_mask_image: 已有纹理区域（strict_compose 会确保完全保留）
                update_image, update_image_before, update_image_after = apply_controlnet_inpaint(
                    inpaint_pipe_sd15,
                    init_image.convert("RGB"),
                    prompt,
                    args.update_strength,
                    ...
                    controlnet_conditioning_scale=float(args.controlnet_strength),
                    strict_compose=True,
                    debug_dir=os.path.join(debug_dir_refine, f"view_{view_idx}_update"),
                )
