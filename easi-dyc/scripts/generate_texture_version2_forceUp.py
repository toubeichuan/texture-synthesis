...
--output_dir outputs_cat_generate_texture_default02_72view
--obj_file cat.obj
...
def _mask_tensor_to_pil(mask_tensor):
    mask_image = mask_tensor[0].detach().float().cpu().permute(2, 0, 1)
    return transforms.ToPILImage()(mask_image).convert("L")
...
if __name__ == "__main__":
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        actual_project_mask_image = actual_generate_mask_image
        # 首视角没有任何已有纹理时，generate_mask 可能因为 similarity/view_threshold
        # 丢掉一些细长可见区域（例如腿），导致黑色 dummy 纹理被直接回投并传播到后续视角。
        # 这时改用完整可见区域 all_mask 做首视角的生成与回投。
        if view_idx == 0 and float(keep_mask_tensor.sum().item()) == 0.0:
            all_visible = float(all_mask_tensor.sum().item())
            generate_visible = float(generate_mask_tensor.sum().item())
            if all_visible > 0 and generate_visible / all_visible < 0.98:
                print(
                    "=> expanding view-0 generate mask from {:.2%} to full visible region".format(
                        generate_visible / all_visible
                    )
                )
                actual_generate_mask_image = _mask_tensor_to_pil(all_mask_tensor)
                actual_project_mask_image = actual_generate_mask_image
        ...
        init_texture, project_mask_image, exist_texture = backproject_from_image(
            ...
            generate_image, actual_project_mask_image, actual_project_mask_image, init_texture, exist_texture, 
        ...
        if not args.no_update and view_idx != 0:
            print(
        ...
        elif not args.no_update and view_idx == 0:
            print("=> skip update for initial view 0")
