...
class ImageUtils:
    ...
    def load_image(img_path, zipfile=None):
        if zipfile is None:
            ...
            if image is None:
                raise FileNotFoundError("The image file cannot be read: {}".format(img_path))
            image = image[:, :, ::-1]  # BGR2RGB
        else:
            image = zipfile.read(img_path)
            image = cv2.imdecode(np.frombuffer(image, np.uint8), cv2.IMREAD_COLOR)
            if image is None:
                raise FileNotFoundError("The image file cannot be read from the compressed package: {}".format(img_path))
            image = image[:, :, ::-1]  # BGR2RGB
        return image
