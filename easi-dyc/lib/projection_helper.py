...
def select_viewpoint(selected_view_ids, view_punishments,
    ...
    device, controlnet_cond, edge_selector, use_principle=False, frozen_update_texture=None
):
    if mode == "sequential":
        ...
        if use_principle and view_idx < 6:
            ...
            for sample_idx in tqdm(range(len(dist_list))):
                ...
                view_heat, *_ = render_one_view_and_build_masks(dist_list[sample_idx], elev_list[sample_idx], azim_list[sample_idx], 
                    ...
                    frozen_update_texture=frozen_update_texture)
...
def build_diffusion_mask(mesh_stuff, 
    ...
    smooth_mask=False, view_threshold=0.01, camera_pos=None, frozen_update_texture=None):
    ...
    # Optionally freeze specific UV texels so they never participate in
    # update-stage diffusion again once they have been claimed.
    if frozen_update_texture is not None:
        mask_mesh.textures = TexturesUV(
            maps=frozen_update_texture.unsqueeze(0).unsqueeze(-1).expand(-1, -1, -1, 3).to(device),
            faces_uvs=faces.textures_idx[None, ...],
            verts_uvs=verts_uvs[None, ...],
            sampling_mode="nearest"
        )
        frozen_mask_tensor, *_ = render(
            mask_mesh, edge_mesh, renderer, controlnet_cond=controlnet_cond, edge_selector=edge_selector, camera_pos=camera_pos
        )
        frozen_mask_tensor = (frozen_mask_tensor > 0).float() * valid_mask_tensor
        update_mask_tensor = update_mask_tensor * (1 - frozen_mask_tensor)

    # keep mask => exist mask - update mask
    old_mask_tensor = exist_mask_tensor - update_mask_tensor
    old_mask_tensor[old_mask_tensor < 0] = 0
