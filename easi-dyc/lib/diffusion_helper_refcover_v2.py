...
def process_inpaint_withref(
    ...
    *,
    ...
    # Merge background only when control map actually uses `depth_pad`
    # as its background marker (typically depth mode).
    has_depth_background = bool((detected_map_np == depth_pad).any())
    background_mask = np.zeros_like(unknown_mask_np, dtype=np.float32)
    if has_depth_background:
        background_mask = (detected_map_np == depth_pad).astype(np.float32) * 255.0
