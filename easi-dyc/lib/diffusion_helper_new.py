...
from lib.reference_attention_new import ensure_ref_unet_wrapped, ReferenceOnlyAttnProc
...
def _prepare_latent_mask(
    ...
    detected_map_resized: np.ndarray,
    latent_shape,
    *,
    device,
    dtype,
    ...
    if unknown_mask is None:
        unknown_mask_np = np.ones(detected_map_resized.shape[:2], dtype=np.uint8) * 255
    else:
        unknown_mask_np = np.asarray(unknown_mask).astype(np.uint8)

    detected_map_np = np.array(Image.fromarray(detected_map_resized).convert("L"))
    if merge_background:
...
def process_controlnet_img2img_withref(
    ...
    *,
    ...
    latent_mask = _prepare_latent_mask(
        unknown_mask,
        detected_map_resized,
        init_latents.shape,
        device=exec_device,
    ...
    try:
        source_start_latents = scheduler.add_noise(init_latents, noise, latent_timestep)
        # For the unknown region, start from pure noise at the chosen timestep
        # instead of from the current render latent. This avoids large unseen
        # regions inheriting the black dummy texture.
        unknown_start_latents = scheduler.add_noise(
            torch.zeros_like(init_latents), noise, latent_timestep
        )
    except Exception:
        source_start_latents = scheduler.add_noise(
            init_latents, noise, latent_timestep.reshape(-1)
        )
        unknown_start_latents = scheduler.add_noise(
            torch.zeros_like(init_latents), noise, latent_timestep.reshape(-1)
        )
    latents = (
        latent_mask * unknown_start_latents
        + (1.0 - latent_mask) * source_start_latents
    )
    ...
    if hasattr(pipe.unet, "set_cond_lat"):
        pipe.unet.set_cond_lat(
            cond_lat, is_cfg_guidance=do_classifier_free_guidance
        )
...
def apply_controlnet_img2img_refattn_depth(
    ...
    *,
    ...
    # Keep non-generate area exact on the final image as a last safety net.
    gen_mask = (
        np.array(generate_mask_image.convert("L")).astype(np.float32) / 255.0
    )[..., None]
    base_np = np.array(base_image).astype(np.float32)
    diff_np = np.array(diffused_image).astype(np.float32)
    diffused_image_np = (
        gen_mask * diff_np + (1.0 - gen_mask) * base_np
    ...
    *,
    ...
    if hasattr(model, "unet") and hasattr(model.unet, "set_cond_lat"):
        ...
        with torch.inference_mode():
            ...
            # VAE 输入空间是 [-1, 1]，纯黑图应为 -1，而不是 0（0 是中灰）。
            zero_tensor = torch.full_like(ref_tensor, -1.0)
